...
import { toast } from 'react-toastify';
import { Link } from 'react-router-dom';
import API from '../../api.js'; // <-- apni file ka correct path use karo
...
const AdminProduct = () => {
  ...
  const fetchProducts = async () => {
    try {
      const res = await API.get('/api/products/get-all-products', { withCredentials: true });
      setProducts(res.data.products);
    } catch (error) {
      console.log("Error fetching products:", error);
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await API.delete(`/api/products/delete-product/${id}`, { withCredentials: true });
      toast.success(res.data.message);
      setProducts(products.filter((p) => p._id !== id));
    } catch (error) {
      console.error("Error deleting product:", error);
